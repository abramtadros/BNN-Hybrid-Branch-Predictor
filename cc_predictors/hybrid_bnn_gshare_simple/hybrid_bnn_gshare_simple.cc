...
std::size_t hybrid_bnn_gshare_simple::gs_table_hash(champsim::address ip, history_type history)
{
...
void hybrid_bnn_gshare_simple::push_history(history_type& history, bool taken)
{
  history <<= 1;
  history.set(0, taken);
}

bool hybrid_bnn_gshare_simple::is_weak_counter(int counter)
{
  return counter == 1 || counter == 2;
}

auto hybrid_bnn_gshare_simple::encode_features(champsim::address ip, history_type history, const base_prediction& base) -> bnn_type::feature_array
{
...
auto hybrid_bnn_gshare_simple::base_predict(champsim::address ip, history_type history) const -> base_prediction
{
...
void hybrid_bnn_gshare_simple::update_base(const base_prediction& prediction, bool taken)
{
  gs_history_table[prediction.index] += taken ? 1 : -1;
}

bool hybrid_bnn_gshare_simple::predict_branch(champsim::address ip)
{
...
void hybrid_bnn_gshare_simple::last_branch_result(champsim::address ip, champsim::address branch_target, bool taken, uint8_t branch_type)
{
